---
title: "Proceedings of the Follow-up Training on R for National Forest Inventory Data Analysis"
#author: "Gael Sola"
#date: "25/08/2021"
output: 
  pdf_document:
...
**Proceedings of the Follow-up Training on R for National Forest Inventory Data Analysis**
...
As part of its technical support to Thailand, the CBIT-Forest project aims to improve transparency
of Thailand National Forest Inventory and its use for national and international reporting. This
includes building on the technical support provided by FAO through the FCPF UTF project (2018-
2020) to further build DNP capacities to analyze and disseminate NFI data, calculate forest carbon
stock estimates at the national level and transpose other DNP routine data analysis to R. During
the UTF project, data analysis was performed with a statistical and programming language, R. At
the end of the UTF project, additional training was requested by DNP to improve its capacity to use
R for NFI data analysis.


## Objectives {-}

The objective of this activity is to strengthen DNP data analysis capacity through R trainings. The specific objectives of the training are:

1. To establish a data processing plan, i.e. listing the type of analyzes commonly implemented by DNP, both at national and subnational level and establishing a plan with the different steps from data collection, digitization and analysis.
1. Refresh participants on the key functions used to calculate the FREL/FRL.
1. Practice advanced functions and code, based on needs, for national or sub-national reporting, to calculate forest indicators beyond Carbon Stock (tidyverse).
1. Demonstrate additional uses of R for reporting (`rmarkdown`) or spatial data analysis (`sf` and `raster` packages).


## Participants {-}

Fifteen participants attended the workshop, all from the Department of National Parks, Wildlife and Plant Conservation (DNP). Five participants attended previous training courses and were familiar with using R in general. The other two third of the participants did not attend previous training, but thanks to the more experienced participants they were able to catch up with the course content. 


## Duration and location{-}

The training was held at DNP from 29 June to 2 July 2021.


## Key lessons learned {-}

Contrary to previous training courses that were very technical and focused on programming, this training also discussed the general data analysis workflow and introduced theoretical elements. The idea behind was to integrate the analysis of Thailand National Forest Inventory for REDD+ monitoring into the general workflow of a multi-purpose national forest inventories. 

After a general introduction on the use of NFI for REDD+ and climate change, the participants discussed and filled a couple of tables on data management to highlight the different operations regarding NFI design and implementation and their timing. 

Session 2 started with an exercise on the general data analysis workflow: Data collection, Data processing, Data cleaning, Exploratory data analysis, Modelling and algorithms, Data product, and Communication. All the other sessions were introduced with there particular place in the workflow. A description of the modification to the FREL/FRL submission was provided and participants practiced the calculation chain from plot data to forest level estimates with MS Excel as it was provided to the FREL/FRL Technical Assessment team.

Session 3 introduced additional R functions to improve R graphs quality for reporting, with an emphasis on labels, highlighting outliers adn grouping several plots in one figure. For this session an R project was created and the full data analysis sequence implemented through various R scripts. 

Session 4 consisted in general considerations and discussions on sampling design. Currently ThaiForm (Thailand National Forest Inventory inherited from a first cycle in 2002 with support from ITTO) is only implemented on protected areas. This will likely not change in the future as DNP budget is not decreasing and their mandate is limited to protected areas.

Session 5 was dedicated to practice a full data analysis sequence and advanced figure production with bamboo. Participants successfully conducted the full chain of calculations from the creation of an R project to the production of report ready figures. 

Finally, R can be used to create reports that integrate formatted text and output of R code. The report creation wand main formatting tips were introduced, however the report is not easily usable for non-latin characters and could not be produced with Thai language. The instruction to use Thai language in the text, figure and tables is presented in annex to the proceedings.
