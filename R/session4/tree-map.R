...
tree_map %>% 
  filter(plotID == "474446662001305C") %>% 
  ggplot() +
  geom_text(aes(x = angle_c3, y = as.numeric(Distance_c3), color = flag_dbh, label = itemNo), size = 3) +
  geom_text(aes(x = angle_c4, y = as.numeric(Distance_c4), color = flag_dbh, label = itemNo), size = 3) +
  geom_hline(yintercept = 17.84, color = "darkred") +
  geom_hline(yintercept = 12.62, color = "darkred") +
  geom_hline(yintercept = 3.99, color = "darkred") +
  scale_x_continuous(
